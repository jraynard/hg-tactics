...
namespace HeavyGear.Shaders
{
    ...
    public class ShaderEffect : IDisposable
    {
        ...
        public void Reload()
        {
            // Load shader — may fail on Linux without WINE-based HLSL compiler.
            try
            {
                effect = BaseGame.Content.Load<Effect>("Shaders/" + shaderContentName);
            }
            catch (Exception ex)
            {
                // Non-fatal: shader features (debug line rendering) will be disabled.
                Log.Write("ShaderEffect.Reload: could not load '" + shaderContentName + "': " + ex.Message);
                effect = null;
                return;
            }
